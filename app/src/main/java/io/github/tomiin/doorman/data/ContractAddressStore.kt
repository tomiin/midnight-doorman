...
// Per-network store of the deployed Doorman registry address.
...
// device dumps.
//
// Key shape: "doorman.<network-name>" (doorman.UNDEPLOYED, doorman.PREPROD).
...
class ContractAddressStore @Inject constructor(
    ...
    private val prefs = EncryptedSharedPreferences.create(
        context,
        "doorman-registry-prefs",
        masterKey,
        EncryptedSharedPreferences.PrefKeyEncryptionScheme.AES256_SIV,
        EncryptedSharedPreferences.PrefValueEncryptionScheme.AES256_GCM,
    )

    private fun keyFor(network: MidnightNetwork): String = "doorman.${network.name}"
